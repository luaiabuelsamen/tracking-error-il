...
media=ROOT/'portfolio/media'; media.mkdir(parents=True,exist_ok=True)
items=[]
...
for source in sorted((ROOT/'results/showcase').glob('*.mp4'),reverse=True):
...
(ROOT/'portfolio/gallery.js').write_text('window.robotGallery = '+json.dumps(items,indent=2)+';\n')
print(f'Gallery updated: {len(items)} recordings. Open portfolio/index.html or serve portfolio/.')
