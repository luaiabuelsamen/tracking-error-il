...
\begin{abstract}
The difference between a commanded joint position and the position actually
reached is available on any position-controlled robot that logs both, with no
added sensor or fitted force model.
...
evaluations that begin after a scripted approach. The benefit varies
substantially across training seeds and does not by itself show that the
policy uses contact information. A study of 546 archived teleoperation
episodes further shows that large tracking error at grasp onset does not
...
For inexpensive robots this is attractive: it needs neither an added sensor nor
a force-estimation model, and it can be reconstructed from existing
demonstrations wherever the logged commands are the goals the servos actually
received. Cheapness is not the question. Tracking error mixes contact with
motion lag and actuator dynamics, and a history of positions may carry the same
information. The question is whether exposing it helps an imitation policy, and
what would justify attributing any gain to contact.
...
Tracking error wins both completed hardware comparisons, by 4 and 13 placements
out of 20, the second significantly. In simulation, position history matches
it, so the results argue for keeping command history in policy interfaces
without establishing that the policy reads contact.
...
and its representation. Position history performs comparably
...
Hardware improvements depend on the trained policy. Table~\ref{tab:hardware}
reports each comparison separately: tracking error improves placement by 20
percentage points for seed 0 and 65 points for seed 1. Exact two-sided McNemar
...
Tracking error costs nothing to add: the command interface already carries
both signals. In simulation and in two hardware comparisons it improved
behavior cloning at a 50-demonstration budget. Whether the policy uses it as
contact information is a separate question that this study leaves open.
Position history is competitive in simulation, no history control has been run
on hardware, and the hardware task never requires staying under a force
threshold.

The two completed hardware seeds gave a 20-point and a 65-point improvement,
and two seeds cannot bound that spread. The study uses one arm, one task and a
fixed scripted approach, so it says nothing about full-task reliability, other
objects or robots, or performance without the command limiter.

The next experiments are a position-history control on hardware, more training
seeds per design, and a task with a measured force limit. Those three settle,
respectively, whether tracking error adds information beyond history, how large
the seed effect is, and whether a policy can use the signal to regulate
contact.
...
no diagnosis of the recurring overload. Trial-level detail is in the
repository's evaluation records.
...
the last 100 steps, median summed joint-coordinate travel, a descriptive sum
over mixed coordinates, is 14.3 for successes
...
could simply follow from progressing further through the task. Traces were
grouped by outcome after the fact and contain no video, object pose or force,
so they cannot say whether a failure was a missed grasp, a slip or a
misplacement, and the endpoint separation is not a validated success detector.
